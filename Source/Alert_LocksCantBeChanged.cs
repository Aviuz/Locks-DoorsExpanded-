...
namespace LocksDoorsExpanded
{
    internal class Alert_LocksCantBeChanged : Alert
    {
        ...
        private IEnumerable<Building_DoorExpanded> UnchangableDoors
        {
            get
            {
                var maps = Find.Maps;
                foreach (var map in maps)
                {
                    ...
                    for (int i = 0; i < desList.Count; i++)
                    {
                        ...
                        {
                            if (LockUtility.GetData(desList[i].target.Thing as Building_DoorExpanded).WantedPrivate && !LockUtility.GetData(desList[i].target.Thing as Building_DoorExpanded).WantedState.owners.Any(p => p.workSettings.WorkIsActive(DefDatabase<WorkTypeDef>.GetNamed("Flicker"))))
